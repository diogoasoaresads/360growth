...
import {
  exchangeGA4AuthCode,
  verifyOAuthState,
} from "@/lib/integrations/providers/ga4";
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  let tokens: Awaited<ReturnType<typeof exchangeGA4AuthCode>>;
  try {
    tokens = await exchangeGA4AuthCode(code);
  } catch {
    return redirectError(redirectBase, "token_exchange_failed");
  }

  const { access_token: _at, refresh_token, scope } = tokens;
  void _at; // access_token intentionally not stored long-term
  ...
  await db
    ...
    .values({
      ...
      status: "connected",
      secretId,
      accountLabel: "Google Analytics 4 (conectado)",
  ...
  await createAuditLog({
    ...
    details: { provider: "GA4", ownerScope },
    ...meta,
  });

  const successUrl = new URL(redirectBase, APP_URL);
  successUrl.searchParams.set("connected", "ga4");
